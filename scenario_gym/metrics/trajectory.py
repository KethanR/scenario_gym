...
class DistanceToEgo(Metric):
    ...
    def _step(self, state: State) -> None:
        for entity in state.get_entities_in_radius(*state.poses[self.ego][:2], 10):
            ...
            # state.t inclusion in the dictionary (line below). To track/timestamp TTC in simulation.
            if state.t not in self.distance_to_ego_dict:
                self.distance_to_ego_dict[state.t] = []
            self.distance_to_ego_dict[state.t].append({"distance_to_ego" : {f"{entity}": distance}})
...
class SafeLongDistance(Metric):
    ...
    def _get_lane_and_lane_center_index(self, entity_pos, entity_road_network_information):
        # Initialize variables to deduce the closest lane center coordinates to entity
        ...
        for lane_index, road_network_type in enumerate(entity_road_network_information[0]):
            if road_network_type == 'Lane':
                ...
                for lane_center_index, coords in enumerate(lane.center.coords):
                    # Compute the Euclidean distance between ego position and lane center
                    distance = abs(np.linalg.norm(entity_pos - coords))
                    # Update the closest lane center index if the distance is smaller
    ...
    def _is_ego_leading_following_or_side(self, pos1, roadinfo1, pos2, roadinfo2):
        ...
        '''
        current_lane_vehicle_1, final_lane_center_index_vehicle_1 = self._get_lane_and_lane_center_index(pos1, roadinfo1)
        current_lane_vehicle_2, final_lane_center_index_vehicle_2 = self._get_lane_and_lane_center_index(pos2, roadinfo2) 

        if current_lane_vehicle_1 == 0 or current_lane_vehicle_2 == 0:
            return "out-of-bounds"
        
        elif current_lane_vehicle_1 == current_lane_vehicle_2:
            current_lane_center_point = np.array(current_lane_vehicle_1.center.coords[final_lane_center_index_vehicle_1])        
            next_lane_center_point = np.array(current_lane_vehicle_1.center.coords[final_lane_center_index_vehicle_1 - 1])            
            longitudinal_direction = -(next_lane_center_point - current_lane_center_point)
            # Project relative position vectors onto the direction vector using dot product
    ...
    def _longitudinal_speed(self, entity_pos, entity_vel, entity_road_network_information):

        current_lane, final_lane_center_index = self._get_lane_and_lane_center_index(entity_pos, entity_road_network_information) 
        ...
        # Because we only need the projection of the entity's velocity onto the longitudinal direction (no sign needed for speed)
        current_lane_center_point = np.array(current_lane.center.coords[final_lane_center_index])        
        next_lane_center_point = np.array(current_lane.center.coords[final_lane_center_index - 1])
        
        longitudinal_direction = next_lane_center_point - current_lane_center_point
        normalised_longitudinal_direction = longitudinal_direction / np.linalg.norm(longitudinal_direction)
    ...
    def _longitudinal_distance(self, pos_ego, pos_non_ego, entity_road_network_information):

        current_lane, final_lane_center_index = self._get_lane_and_lane_center_index(pos_ego, entity_road_network_information) 
        ...
        longitudinal_distance = np.dot(relative_position, normalised_longitudinal_direction)        
    ...
    def _calculate_safe_long_dist(self, speed_rear, speed_front):
        ...
        safeLonDis = 0.5 * np.power(speed_front, 2) / self.accFrontMaxBrake - speed_rear * self.responseTime - 0.5 * \
                        np.power(self.responseTime, 2) * self.accRearMaxResp - 0.5 * sign * np.power(speed_rear + self.responseTime *
                                                                            self.accRearMaxResp, 2) / self.accRearMinBrake
        safeLonDisBrake = 0.5 * np.power(speed_front, 2) / self.accFrontMaxBrake - speed_rear * self.responseTime - 0.5 * \
                            np.power(self.responseTime, 2) * self.accRearMaxResp - 0.5 * sign * np.power(speed_rear + self.responseTime *
                                                                            self.accRearMaxResp, 2) / self.accRearMaxBrake
    ...
    def _step(self, state: State) -> None:
        ...
        if len(state.get_entities_in_radius(*state.poses[self.ego][:2], 10)) > 1:
            for entity in state.get_entities_in_radius(*state.poses[self.ego][:2], 10):
                ...
                ego_relative_position_string = self._is_ego_leading_following_or_side(pos_ego, ego_road_network_info, pos_non_ego, non_ego_road_network_info)

                if ego_relative_position_string == "following":
                    speed_rear = self._longitudinal_speed(pos_ego, vel_ego, ego_road_network_info)
                    speed_front = self._longitudinal_speed(pos_non_ego, vel_non_ego, non_ego_road_network_info)
                elif ego_relative_position_string == "leading":
                    speed_rear = self._longitudinal_speed(pos_non_ego, vel_non_ego, non_ego_road_network_info)
                    speed_front = self._longitudinal_speed(pos_ego, vel_ego, ego_road_network_info)
                else:
                    continue

                safeLonDis, safeLonDisBrake = self._calculate_safe_long_dist(speed_rear, speed_front)
                long_dist = self._longitudinal_distance(pos_ego, pos_non_ego, ego_road_network_info)

                long_risk = self._long_risk_index(safeLonDis, safeLonDisBrake, long_dist)

                # state.t inclusion in the dictionary (line below). To track/timestamp TTC in simulation.
                if state.t not in self.safe_long_dist_dict:
                    self.safe_long_dist_dict[state.t] = []
                self.safe_long_dist_dict[state.t].append({"safe_longitudinal_distance" : {f"{entity}": safeLonDis}})
                
                if state.t not in self.safe_long_dist_brake_dict:
                    self.safe_long_dist_brake_dict[state.t] = []
                self.safe_long_dist_brake_dict[state.t].append({"safe_longitudinal_distance_brake" : {f"{entity}": safeLonDisBrake}})

                if state.t not in self.long_dist_dict:
                    self.long_dist_dict[state.t] = []
                self.long_dist_dict[state.t].append({"longitudinal_distance" : {f"{entity}": long_dist}})

                if state.t not in self.long_risk_dict:
                    self.long_risk_dict[state.t] = []
                self.long_risk_dict[state.t].append({"longitudinal_risk" : {f"{entity}": long_risk}})
                

    def get_state(self) -> float:
        """Return the current distance travelled."""
        return self.safe_long_dist_dict, self.safe_long_dist_brake_dict, self.long_dist_dict, self.long_risk_dict
...
class SafeLatDistance(Metric):
    ...
    def _latitudinal_speed(self, entity_pos, entity_vel, entity_road_network_information):

        current_lane, final_lane_center_index = self._get_lane_and_lane_center_index(entity_pos, entity_road_network_information) 

        # Convert closest lane center point and next-nearest lane center to array
        # Does not matter if the next lane center point is 'ahead' or 'behind' direction of travel...
        # Because we only need the projection of the entity's velocity onto the latitudinal direction (no sign needed for speed)
        current_lane_center_point = np.array(current_lane.center.coords[final_lane_center_index])        
        next_lane_center_point = np.array(current_lane.center.coords[final_lane_center_index - 1])
        
        longitudinal_direction = -(next_lane_center_point - current_lane_center_point)
        
        latitudinal_direction = np.array([-longitudinal_direction[1], longitudinal_direction[0]])
        normalised_latitudinal_direction = latitudinal_direction / np.linalg.norm(latitudinal_direction)
    ...
    def _latitudinal_distance(self, pos_ego, pos_non_ego, entity_road_network_information):
        ...
        latitudinal_distance = np.dot(relative_position, normalised_latitudinal_direction)        
    ...
    def _calculate_safe_lat_dist(self, speed_left, speed_right):
        ...
        safeLatDis = 0.5 * (speed_left + vLeft) * self.responseTime + 0.5 * sign_left * np.power(vLeft, 2) / acc_left_min_gap - (
                0.5 * (speed_right + vRight) * self.responseTime + 0.5 * sign_right * np.power(vRight, 2) / acc_right_min_gap)
        safeLatDisBrake = 0.5 * (speed_left + vLeft) * self.responseTime + 0.5 * sign_left * np.power(vLeft, 2) / \
                        acc_left_max_gap - (0.5 * (speed_right + vRight) * self.responseTime + 0.5 * sign_right *
    ...
    def _step(self, state: State) -> None:
        ...
        if len(state.get_entities_in_radius(*state.poses[self.ego][:2], 10)) > 1:
            for entity in state.get_entities_in_radius(*state.poses[self.ego][:2], 10):
                ...
                check = self._is_ego_left_or_right(pos_ego, ego_lane, pos_non_ego, non_ego_lane)
                
                if check == None or check == "inline":
                    continue
                elif check == "right":
                    speed_right = self._latitudinal_speed(pos_ego, vel_ego, ego_lane)
                    speed_left = self._latitudinal_speed(pos_non_ego, vel_non_ego, non_ego_lane)
                elif check == "left":
                    speed_right = self._latitudinal_speed(pos_non_ego, vel_non_ego, non_ego_lane)
                    speed_left = self._latitudinal_speed(pos_ego, vel_ego, ego_lane)
                else:
                    continue

                safeLatDis, safeLatDisBrake = self._calculate_safe_lat_dist(speed_left, speed_right)
                lat_dist = self._latitudinal_distance(pos_ego, pos_non_ego, ego_lane)

                lat_risk = self._lat_risk_index(safeLatDis, safeLatDisBrake, lat_dist)

                # state.t inclusion in the dictionary (line below). To track/timestamp TTC in simulation.
                if state.t not in self.safe_lat_dist_dict:
                    self.safe_lat_dist_dict[state.t] = []
                self.safe_lat_dist_dict[state.t].append({"safe_latitudinal_distance" : {f"{entity}": safeLatDis}})

                if state.t not in self.safe_lat_dist_brake_dict:
                    self.safe_lat_dist_brake_dict[state.t] = []
                self.safe_lat_dist_brake_dict[state.t].append({"safe_latitudinal_distance_brake" : {f"{entity}": safeLatDisBrake}})

                if state.t not in self.lat_dist_dict:
                    self.lat_dist_dict[state.t] = []
                self.lat_dist_dict[state.t].append({"latitudinal_distance" : {f"{entity}": lat_dist}})

                if state.t not in self.lat_dist_dict:
                    self.lat_dist_dict[state.t] = []
                self.lat_dist_dict[state.t].append({"latitudinal_risk" : {f"{entity}": lat_risk}})

    def get_state(self) -> float:
        """Return the current distance travelled."""
        return self.safe_lat_dist_dict, self.safe_lat_dist_brake_dict, self.lat_dist_dict, self.lat_risk_dict
...
class TimeToCollision(Metric):
    ...
    def _will_collide_check_and_ttc(self, pos1, vel1, box1, pos2, vel2, box2, distance_tolerance, time_horizon=5, time_steps=50):
        x1, y1 = pos1
        vx1, vy1 = vel1
        x2, y2 = pos2
        vx2, vy2 = vel2
        ...
        for t in times:
            ...
            box1_at_t = self._translate_polygon(box1, x1 + vx1*t - x1, y1 + vy1*t - y1)
            box2_at_t = self._translate_polygon(box2, x2 + vx2*t - x2, y2 + vy2*t - y2)
            ...
            # If either of these conditions are true, then 't' is the time-to-collision (TTC).   
            if box1_at_t.intersects(box2_at_t) or box1_at_t.distance(box2_at_t) <= distance_tolerance:
                return [True, t]
        return [False, None]
    ...
    def _step(self, state: State) -> None:
        ...
        # Get entities within 10m of the ego, if entities exist... 
        if len(state.get_entities_in_radius(*state.poses[self.ego][:2], 10)) > 1:
            # Then loop through non_ego entities and check if they will reach intersection point at approx. the same time as the ego (function _will_collide does this).
            for entity in state.get_entities_in_radius(*state.poses[self.ego][:2], 10):
                if entity == self.ego:
                    continue
                ...
                self.ego_time_to_collision_bool_and_value = self._will_collide_check_and_ttc(pos_ego, vel_ego, box_ego, pos_non_ego, vel_non_ego, box_non_ego, distance_tolerance)
                
                # If they will collide, then retrieve time-to-collision. 
                if self.ego_time_to_collision_bool_and_value[0] == True: 
                    # state.t inclusion in the dictionary (line below). To track/timestamp TTC in simulation.
                    if state.t not in self.time_to_collision_dict:
                        self.time_to_collision_dict[state.t] = []
                    self.time_to_collision_dict[state.t].append({"time_to_collision" : {f"{entity}": self.ego_time_to_collision_bool_and_value[1]}})
                else:
...
class SpaceOccupancyIndex(Metric):
    ...
    def _step(self, state: State) -> None:
        ...
        if len(state.get_entities_in_radius(*state.poses[self.ego][:2], self.predefined_radius)) > 1:
            for entity in state.get_entities_in_radius(*state.poses[self.ego][:2], self.predefined_radius):
                if entity == self.ego:
                    continue
                ...
                self.space_occupancy_index[state.t].append({"space_occupancy_index" : {f"{entity}": 1}})
...
class DeltaV(Metric):
    ...
    def _will_collide_check(self, pos1, vel1, box1, pos2, vel2, box2, distance_tolerance, time_horizon=5, time_steps=50):
        x1, y1 = pos1
        vx1, vy1 = vel1
        x2, y2 = pos2
        vx2, vy2 = vel2
    ...
    def _step(self, state: State) -> None:
        ...
        if len(state.get_entities_in_radius(*state.poses[self.ego][:2], 10)) > 1:
            ...
            for entity in state.get_entities_in_radius(*state.poses[self.ego][:2], 10):
                ...
                # Check if ego will collide with other entity. 
                self.ego_time_to_collision_bool_and_value = self._will_collide_check(pos_ego, vel_ego, box_ego, pos_non_ego, vel_non_ego, box_non_ego, distance_tolerance)

                # If they will collide, then retrieve time-to-collision. 
                if self.ego_time_to_collision_bool_and_value[0] == False:
                    continue
                    ...
                    entity_delta_v = (self.predefined_masses[ego_class] / (self.predefined_masses[ego_class] + self.predefined_masses[entity_class])) * (np.linalg.norm(vel_ego) - np.linalg.norm(vel_non_ego))

                    # state.t inclusion in the dictionary (line below). To track/timestamp TTC in simulation.
                    if state.t not in self.delta_v_dict:
                        self.delta_v_dict[state.t] = []
                    self.delta_v_dict[state.t].append({"delta_v" : {f"{entity}": entity_delta_v}})
                else:
                    continue 
...
class CollisionCheck(Metric):
    ...
    def _step(self, state: State) -> None:
        if self.collision_check_and_timestamp == False:
